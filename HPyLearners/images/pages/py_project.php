...
<body>
	...
	?>
		...
		<div class="plang_top_body1 container-fluid">
		...
		<?php  
			$get_sl_sql = "SELECT Serial_no FROM videos WHERE videos.`PageName` = 'Python_project'";
			$get_sl_query = mysqli_query($con,$get_sl_sql);


			if (isset($_POST['go_video'])) {
				$serial_no = $_POST['vid_sl_num'];
				$_SESSION['sl_no_02'] = $serial_no;
			...
			if (isset($_POST['next'])) {
				$get_last_sl_sql = "SELECT Serial_no FROM videos WHERE videos.`PageName` = 'Python_project' ORDER BY Serial_no DESC";
			...
			if (isset($_POST['Com_submit'])) {
				...
				// echo "Serial no pai na = ".$_SESSION['sl_no_02'];
				$serial_no = $_SESSION['sl_no_02'];
				...
				$CinsertQuery = "INSERT INTO `video_comments` (`Date`, `Email`, `Comment`, `Serial_no`, `Page_Name`, `id`) VALUES ('$cur_date', '$CommenterEmail', '$py_comment', '$serial_no', 'Python_project', NULL)";
			...
			if ($serial_no==NULL or $_SESSION['sl_no_02']!='') {
				$serial_no = $_SESSION['sl_no_02'];
			}
			

			$video_selectSql = "SELECT Serial_no, Video_Name, Link, Code_link, Description FROM videos WHERE videos.`PageName` = 'Python_project' AND videos.`Serial_no`='$serial_no'";
	...
	<?php 
		...
		$cmnt_select = "SELECT * FROM `video_comments` WHERE Page_Name='Python_project' AND Serial_no='$serial_no' ORDER BY id DESC";
