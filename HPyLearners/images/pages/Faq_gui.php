...
<body>
	...
	$faq_select_sql545 = "SELECT * FROM `faq` WHERE `Topic` = 'Frame works' ORDER BY Faq_serial DESC";
	...
	<div class="Faq_mainBody">
		...
		<nav> 
			<div class="Faq_nav"> 
				...
				<li><a href="Faq_gui.php" id="faq_menu_active">Frame Work</a></li>
	...
	<?php  
		if (isset($_POST['go_to_top'])) {
			$faq_select_sql = "SELECT * FROM `faq` WHERE `Topic` = 'Frame works' ORDER BY Faq_serial DESC";
			$faq_select_query = mysqli_query($con,$faq_select_sql);
			$top_serial = mysqli_num_rows($faq_select_query);
			$_SESSION['top_sl2'] = $top_serial;
		}
		...
		if (isset($_POST['faq_go_btn'])) {
			...
			$_SESSION['top_sl2'] = $top_serial;
			$faq_select_sql = "SELECT * FROM `faq` WHERE `Topic` = 'Frame works' AND Faq_serial='$top_serial'";
			$faq_select_query = mysqli_query($con,$faq_select_sql);
			$faq_selected_row = mysqli_fetch_array($faq_select_query);
		...
		if (isset($_POST['Faq_com_submit'])) {
			...
			$selectAll_commenter_sql = "SELECT id FROM faq_comment WHERE Faq_serial = '$top_serial' AND Topic='Frame works' ORDER BY id DESC";
			...
			$faq_comment_insert_sql = "INSERT INTO `faq_comment` VALUES('$res[2]','$cur_date','$top_serial','Frame works', '$faq_com', '$id_896')";
			...
			$body = "Hi {$mentioned_name[0]}, {$res[0]} {$res[1]} mentioned you in a comment on Faq = {$id_896}, Topic = Frame works";
		...
		elseif($_SESSION['top_sl2']!=NULL){

			$top_serial = $_SESSION['top_sl2'];
			$faq_select_sql = "SELECT * FROM `faq` WHERE `Topic` = 'Frame works' AND Faq_serial='$top_serial'";
			$faq_select_query = mysqli_query($con,$faq_select_sql);
			$faq_selected_row = mysqli_fetch_array($faq_select_query);
		}
...
<?php  
	...
	if (isset($_POST['del_post'])) {
		...
		$del_post_sql = "DELETE FROM `faq` WHERE `faq`.`Faq_serial` = '$faq_serial_no' AND `faq`.`Topic` = 'Frame works'";
	...
	if (isset($_POST['edit_post'])) {
		...
		$update_post_sql = "UPDATE `faq` SET `faq_Question` = '$edited_post' WHERE `faq`.`Faq_serial` = '$faq_serial_no' AND `faq`.`Topic` = 'Frame works'";
