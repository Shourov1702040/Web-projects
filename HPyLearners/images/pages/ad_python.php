...
<body>
	...
	?>
		...
		<div class="plang_header container">
			...
			<b><p><u>Welcome to our Basic python tutorial</u></p></b>
		</div>
			
		<div class="plang_top_body container-fluid">
		...
		<?php  
			$get_sl_sql = "SELECT Serial_no FROM videos WHERE videos.`PageName` = 'Advance_python'";
			$get_sl_query = mysqli_query($con,$get_sl_sql);


			if (isset($_POST['go_video'])) {
				$serial_no = $_POST['vid_sl_num'];
				$_SESSION['sl_no_01'] = $serial_no;
			...
			if (isset($_POST['next'])) {
				$get_last_sl_sql = "SELECT Serial_no FROM videos WHERE videos.`PageName` = 'Advance_python' ORDER BY Serial_no DESC";
			...
			if (isset($_POST['Com_submit'])) {
				...
				// echo "Serial no pai na = ".$_SESSION['sl_no_01'];
				$serial_no = $_SESSION['sl_no_01'];
				...
				$CinsertQuery = "INSERT INTO `video_comments` (`Date`, `Email`, `Comment`, `Serial_no`, `Page_Name`, `id`) VALUES ('$cur_date', '$CommenterEmail', '$py_comment', '$serial_no', 'Advance_python', NULL)";
			...
			if ($serial_no==NULL or $_SESSION['sl_no_01']!='') {
				$serial_no = $_SESSION['sl_no_01'];
			}
			

			$video_selectSql = "SELECT Serial_no, Video_Name, Link, Code_link, Description FROM videos WHERE videos.`PageName` = 'Advance_python' AND videos.`Serial_no`='$serial_no'";
		...
		<div class="plang_body1 container">
			...
			<div class="vid_show_vid row">
				<?php 
				 ...
				 ?>
				<iframe src="<?php echo $new_vid_embed_link; ?>" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
			</div>
			<div class=" row">
				<div class="vid_show col-md-6">
					<h3><?php echo $current_videdo_row[0].". ".$current_videdo_row[1]; ?></h3>
				</div>
			...
			<div class="mqc_offer">
				<p>Try simple mcq and test yourself <a href="Mcq.php?mcq_id=<?php echo "Advance_python@@".$serial_no; ?>">Test</a></p>
			</div>
	...
	<?php 
		...
		$cmnt_select = "SELECT * FROM `video_comments` WHERE Page_Name='Advance_python' AND Serial_no='$serial_no' ORDER BY id DESC";
		$cmntQuery = mysqli_query($con,$cmnt_select);
		$numOfComment = mysqli_num_rows($cmntQuery);

		while ($res_cmnt = mysqli_fetch_array($cmntQuery)) {
			
			$ck_Em = $res_cmnt['Email'];
			$imgFindQuery = "SELECT Image,FirstName,LastName,Email FROM user_account WHERE Email='$ck_Em'";
			$FindQuery = mysqli_query($con,$imgFindQuery);
			$resIm = mysqli_fetch_array($FindQuery);

			$image = $resIm['Image'];
			$image_src = "upload/".$image;
		?>
			<div class="pylang_comments container row">
				<div class="col-md-2">
					<img src="<?php echo $image_src; ?>">
				</div>
				<div class="col-md-10">
					<div class="row">
						<h4><?php echo $resIm[1]." ".$resIm[2]; ?></h4>
						<h5><?php echo "@".$res_cmnt['Date']; ?></h5>
						<?php 

							$commenter_mail = $resIm["Email"];
							$replay_name = $resIm[1]." ".$resIm[2];

							echo '<button type="button" onclick="replay(\''.$replay_name.'\',\''.$commenter_mail.'\');" class="replay_on_video btn btn-primary" >replay</button>';
						 ?>
					</div>
					<div class="com_faltu row">
						
						<p><?php echo $res_cmnt['Comment']; ?></p>
