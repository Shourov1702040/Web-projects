...
<body>
	...
	$faq_select_sql545 = "SELECT * FROM `faq` WHERE `Topic` = 'Python' ORDER BY Faq_serial DESC";
	...
	<div class="Faq_mainBody">
		...
		<nav> 
			<div class="Faq_nav"> 
				<ul>
				<li><a href="#" id="faq_menu_active">Laungage</a></li>
	...
	<?php  
		if (isset($_POST['go_to_top'])) {
			$faq_select_sql = "SELECT * FROM `faq` WHERE `Topic` = 'Python' ORDER BY Faq_serial DESC";
			$faq_select_query = mysqli_query($con,$faq_select_sql);
			$top_serial = mysqli_num_rows($faq_select_query);
			$_SESSION['top_sl'] = $top_serial;
		}
		...
		if (isset($_POST['faq_go_btn'])) {
			...
			$_SESSION['top_sl'] = $top_serial;
			$faq_select_sql = "SELECT * FROM `faq` WHERE `Topic` = 'Python' AND Faq_serial='$top_serial'";
			$faq_select_query = mysqli_query($con,$faq_select_sql);
			$faq_selected_row = mysqli_fetch_array($faq_select_query);
		...
		if (isset($_POST['Faq_com_submit'])) {
			...
			$selectAll_commenter_sql = "SELECT id FROM faq_comment WHERE Faq_serial = '$top_serial' AND Topic='Python' ORDER BY id DESC";
			...
			$faq_comment_insert_sql = "INSERT INTO `faq_comment` VALUES('$res[2]','$cur_date','$top_serial','Python', '$faq_com', '$id_896')";
		...
		elseif($_SESSION['top_sl']!=NULL){

			$top_serial = $_SESSION['top_sl'];
			$faq_select_sql = "SELECT * FROM `faq` WHERE `Topic` = 'Python' AND Faq_serial='$top_serial'";
			$faq_select_query = mysqli_query($con,$faq_select_sql);
			$faq_selected_row = mysqli_fetch_array($faq_select_query);
		}
...
<?php  
	...
	if (isset($_POST['del_post'])) {
		...
		$del_post_sql = "DELETE FROM `faq` WHERE `faq`.`Faq_serial` = '$faq_serial_no' AND `faq`.`Topic` = 'Python'";
	...
	if (isset($_POST['edit_post'])) {
		...
		$update_post_sql = "UPDATE `faq` SET `faq_Question` = '$edited_post' WHERE `faq`.`Faq_serial` = '$faq_serial_no' AND `faq`.`Topic` = 'Python'";
